...
export default function ProductDetailPage() {
  ...
  if (!product) {
    return <div className="p-10 text-center text-white">Товар загружается или не найден...</div>;
  }

  if (isLoading || !mappedProduct) {
    return (
      <section className="min-h-screen bg-[#0F0F0F] px-4 pt-40 text-white">
        <div className="container-shell grid min-h-[420px] place-items-center">
          <div className="grid justify-items-center gap-4">
            ...
            <p className="text-sm font-bold uppercase tracking-widest text-white/55">Загрузка товара</p>
          </div>
        </div>
      </section>
    );
    ...
    return (
      <section className="min-h-screen bg-[#0F0F0F] px-4 pt-40 text-white">
        <div className="container-shell">
          ...
          <div className="mt-8 rounded-md border border-white/10 bg-white/5 p-8">
  ...
  return (
    <section className="min-h-screen bg-[#0F0F0F] bg-[linear-gradient(rgba(255,255,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.045)_1px,transparent_1px)] bg-[size:42px_42px] px-4 pb-20 pt-36 text-white">
      <div className="container-shell">
        <Link className="inline-flex items-center gap-2 text-sm font-black uppercase text-[#FF4D00]" href="/catalog">
          <ArrowLeft size={17} aria-hidden />
          В каталог
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(380px,0.95fr)] lg:items-start">
          {/* Ensure there is a parent container for the left column */}
          <div className="flex w-full flex-col gap-4 md:w-1/2">
            {/* 1. White Image Box Container */}
            ...
            {/* 2. Description Box placed perfectly right underneath */}
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
              <h3 className="mb-3 text-lg font-medium text-white">Описание товара</h3>
          ...
          <div className="rounded-md border border-white/10 bg-[#151515]/95 p-5 shadow-2xl shadow-black/25 md:p-7">
            <div className="flex flex-wrap gap-2">
              <span className="rounded-md border border-[#FF4D00]/30 bg-[#FF4D00]/10 px-3 py-1 text-xs font-black uppercase text-[#FF4D00]">
                {product?.category}
              </span>
              {product?.subcategory && (
                <span className="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-xs font-black uppercase text-white/70">
                  {product?.subcategory}
                </span>
              )}
            </div>
            ...
            <div className="mt-7 flex flex-col gap-4 border-y border-white/10 py-6 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-widest text-white/38">Цена</p>
                <p className="mt-1 text-4xl font-black text-[#FF4D00]">
                  {Number(product?.price ?? 0).toLocaleString("ru-RU")} KGS
                </p>
              </div>
              <div className="inline-flex items-center gap-2 text-sm font-bold text-green-300">
            ...
            <div className="mt-6">
              <h2 className="text-sm font-black uppercase tracking-widest text-white/45">Характеристики</h2>
              <dl className="mt-4 grid gap-2 sm:grid-cols-2">
                {specs.map((spec) => (
                  <div className="rounded-md border border-white/10 bg-white/[0.035] p-3" key={`${spec.label}-${spec.value}`}>
                    <dt className="text-[11px] font-black uppercase text-white/36">{spec.label}</dt>
                    <dd className="mt-1 text-sm font-bold text-white/82">{spec.value}</dd>
                  </div>
                ))}
              </dl>
            </div>
