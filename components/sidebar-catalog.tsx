...
import { categoryHref, categoryTranslationKey, subcategoryHref, subcategoryMap, subcategoryTranslationKey } from "@/lib/catalog";
  ...
  return (
    <nav className="grid gap-1" aria-label={t("sidebar.ariaLabel")}>
      {catalogItems.map((item) => {
        ...
        const titleKey = item.category ? categoryTranslationKey(item.category) : "";
        const title = titleKey ? t(titleKey, { defaultValue: item.title }) : item.title;
              ...
              <AnimatePresence initial={false}>
                {expanded && hasSubcategories && item.category && (
                  <motion.button
                    ...
                    aria-label={`${subcategoriesExpanded ? t("sidebar.collapseCategory") : t("sidebar.expandCategory")} ${title}`}
                ...
                >
                  <div className="grid gap-1 py-1">
                    {subcategories.map((subcategory, index) => {
                      ...
                      const subcategoryKey = subcategoryTranslationKey(subcategory);
                      const subcategoryTitle = subcategoryKey
                        ? t(subcategoryKey, { defaultValue: subcategory })
                        : subcategory;
                      const subcategoryLabel =
                        item.category === "Котлы"
                          ? t("catalog.boilerSubcategoryLabel", { subcategory: subcategoryTitle })
                          : subcategoryTitle;
                      ...
                      return (
                        <Link
                          ...
                          key={`${item.key}-subcategory-${index}-${subcategory}`}
                          onClick={onNavigate}
                        >
                          {subcategoryLabel}
                        </Link>
  ...
  return (
    <>
      <motion.aside
        ...
        className="fixed bottom-0 left-0 top-28 z-[998] hidden overflow-hidden border-r border-black/10 bg-white px-2 py-4 text-[#151515] shadow-2xl shadow-black/10 dark:border-white/5 dark:bg-[#111111] dark:text-white lg:block"
        initial={false}
      ...
      <button
        className="fixed bottom-24 left-4 z-[998] grid size-12 place-items-center rounded-md bg-[#FF4D00] text-white shadow-[0_0_24px_rgba(255,77,0,0.28)] lg:hidden"
      ...
      <AnimatePresence>
        {isMobileOpen && (
          <>
            <motion.button
              animate={{ opacity: 1 }}
              className="fixed inset-0 z-[998] bg-black/65 backdrop-blur-sm lg:hidden"
              exit={{ opacity: 0 }}
              initial={{ opacity: 0 }}
            ...
            <motion.aside
              animate={{ x: 0 }}
              className="fixed bottom-0 left-0 top-0 z-[999] w-[min(86vw,320px)] border-r border-black/10 bg-white bg-[linear-gradient(rgba(15,15,15,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(15,15,15,0.045)_1px,transparent_1px)] bg-[size:42px_42px] p-4 text-[#151515] shadow-2xl dark:border-white/10 dark:bg-[#111111] dark:bg-[linear-gradient(rgba(255,255,255,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.045)_1px,transparent_1px)] dark:text-white lg:hidden"
              exit={{ x: "-100%" }}
              initial={{ x: "-100%" }}
