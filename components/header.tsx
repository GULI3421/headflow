...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-zinc-800 bg-black text-white shadow-xl shadow-black/20">
      <div className="border-b border-zinc-800/60 bg-zinc-900/50 px-4 py-2 text-xs text-zinc-400">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <a className="hidden items-center gap-2 whitespace-nowrap font-semibold text-zinc-300 transition hover:text-white sm:inline-flex" href="tel:+996708605281">
      ...
      <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-3 px-4 py-3 lg:gap-5">
        <Link href="/" className="flex min-w-fit items-center gap-3" aria-label={t("common.brand")}>
          ...
          <div className="block leading-none">
            <span className="block text-base font-black uppercase text-white sm:text-xl">ЦЕНТР</span>
        ...
        <nav className="hidden shrink-0 items-center gap-0.5 xl:flex">
          {menu.map((item) => (
            <Link
              className="rounded-md px-2.5 py-2 text-sm font-semibold text-white/78 transition hover:bg-white/10 hover:text-white"
              href={item.href}
              key={item.href}
            >
              {t(item.labelKey)}
        ...
        <form className="relative hidden min-w-[240px] flex-1 md:block xl:max-w-2xl" onSubmit={handleSearch}>
          ...
          <input
            ...
            className="h-12 w-full rounded-md border border-zinc-700 bg-zinc-900/75 pl-11 pr-4 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-[#FF4F00] focus:bg-zinc-900"
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Поиск отопительного оборудования..."
            value={query}
          />
          {suggestions.length > 0 && (
            <div className="absolute left-0 right-0 top-14 overflow-hidden rounded-md border border-zinc-800 bg-zinc-950 text-white shadow-2xl">
              {suggestions.map((product) => (
                <Link
                  className="flex items-center justify-between px-4 py-3 text-sm transition hover:bg-zinc-900"
                  href={`/product/${product.id}`}
                  key={product.id}
                >
                  <span className="font-semibold">{product.name}</span>
                  <span className="text-xs text-zinc-500">{product.power}</span>
                </Link>
              ))}
            </div>
          )}
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Link className="relative grid size-10 place-items-center rounded-md bg-white/10 transition hover:bg-white/15" href="/favorites" aria-label="Избранное">
          ...
          <Link className="relative grid size-10 place-items-center rounded-md bg-white/10 transition hover:bg-white/15" href="/cart" aria-label={t("header.cart")}>
        ...
        <button
          className="grid size-10 place-items-center rounded-md bg-white/10 transition hover:bg-white/15 xl:hidden"
      ...
      {menuOpen && (
        <div className="border-t border-zinc-800 bg-black px-4 pb-5 text-white xl:hidden">
          <div className="mx-auto grid max-w-7xl gap-2 py-2">
            <form className="relative my-2 md:hidden" onSubmit={handleSearch}>
              <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" size={18} />
              <input
                aria-label={t("header.searchLabel")}
                className="h-12 w-full rounded-md border border-zinc-700 bg-zinc-900/75 pl-11 pr-4 text-sm text-white outline-none transition placeholder:text-zinc-500 focus:border-[#FF4F00]"
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Поиск отопительного оборудования..."
                value={query}
              />
            ...
            <div className="mt-2 border-t border-white/10 pt-3">
              <Link className="block rounded-md py-2 text-sm font-semibold text-white/70" href="/about">
                {t("header.about")}
              </Link>
              <Link className="block rounded-md py-2 text-sm font-semibold text-white/70" href="/contacts">
                {t("header.contacts")}
              </Link>
